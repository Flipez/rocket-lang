...
func TestFileObjectMethods(t *testing.T) {
	tests := []inputTestCase{
		...
		{`open("../fixtures/module.rl", "r", "0644").content(1)`, "to many arguments: want=0, got=1"},
		{`open("../fixtures/module.rl", "r", "0644").read()`, "to few arguments: want=1, got=0"},
		...
		{`(open("../fixtures/module.rl", "r", "0644").wat().lines().size() == open("../fixtures/module.rl").methods().size() + 1).plz_s()`, "true"},
		{`open("").type()`, "ERROR"},
